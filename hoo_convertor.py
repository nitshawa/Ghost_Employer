...
replacements = open('replacements.json').read()
...
def main_test(value):
    assert type(value) == str, "Invalid type of value {}".format(type(value))
    value = value.lower()
    value = value.strip(' ,\n')

    value = value.replace("\n", " ").replace("\\n", " ")

    validated_check = validate_input_string(value) # check if invalid words are not present in value
