...
replacements = {
    ...
    'wed': ['wednesday', 'mittwoch', 'mercredi', 'miércoles', 'wed-wed', 'wen','we ','we:'],
    'thu': ['thursday', 'donnerstag', 'jeudi', 'jueves', 'thurs', 'thur','thr', 'thu-thu', 'th ', 'th:'],
    'fri': ['friday', 'freitag', 'vendredi', 'viernes', 'fir', 'fri-fri',  'fr ',  'fr:'],
    'sat': ['saturday', 'samstag', 'samedi', 'sábado', 'sat-sat', 'sa ', 'sa:'],
    ...
    ' to ': [' à '],
    ', ': ['/']
    'mon-sun' : ['every day', 'everyday', 'all week', '7 days a week', 'seven days a week', '7 days', 'per day'],
    '[]': ['closed - closed', 'closed-closed', 'close - close', 'close-close', 'closed', 'close'],
    'mon-fri': ['weekday_hours', 'weekday', 'weekdays'],
    '': ['_hours', 'hrs', 'black'],
    '&': ['\u0026'],
    '00:00,00:00': ['24:00rs', '24:00urs', '24:00urs', '24:00s', '24 hours'],
     '-': ['through', 'to', 'thuough', '\xe2\x80\x93', ' – ', '_'],
       # '00:00': ['24:00'],
       # '00:30': ['24:30'],
...
if __name__ == "__main__":
    output_csv = csv.writer(open('Buffalo Wild Wings store_hours.csv', 'wb'))
    output_csv.writerow(['input_string', 'output_dict', "unmatched"])
    # query = """
    #    SELECT * FROM O_O_DATA.O_O_DATA_HOP
    #    WHERE BrandName LIKE "%Buffalo Wild Wings%"
    #    AND Country = "US";
    # """
    query = """
       SELECT * FROM scrapers.locations
       WHERE brand_name LIKE "%uffalo%"
       AND country_code = "US";
...
accuracy_csv = csv.reader(open('Buffalo Wild Wings store_hours.csv', 'rb'))
accuracy_csv = filter(None,[x for x in accuracy_csv][1:])

total  = len([x for x in accuracy_csv])
print ("Total rows %d" % total)
unhandled = len([x for x in accuracy_csv if  'Unhandled' in x[1]])
if unhandled > 0:
    per_accuracy = ((total - unhandled)* 100)/total
    print ("Accuracy  %d" % per_accuracy)
else:
    per_accuracy = "not found"
print ("Unhandled %d" % unhandled)


unhandled_csv = csv.writer(open('unhandled_store_hours.csv', 'wb'))
