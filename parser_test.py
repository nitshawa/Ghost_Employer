...
replacements = {
    ...
    'wed': ['wednesday', 'mittwoch', 'mercredi', 'miércoles', 'wed-wed', 'wen','we '],
...
def replace_hours_for_match(matchobj):
    ...
    ampm = obj.get('ampm', '')
    if ampm == 'pm' and ampm == 'p':
        int_hour = (int_hour + 12) % 24
    print int_hour
    if int_hour == 12 and ampm == 'am' or ampm == 'a':
        int_hour = 00
    if int_hour > 23:
        int_hour = 00
...
def time_day_dict(value):
    ...
    for  hour in hours:
        ...
        # print raw_days
        days = re.sub(r'[\[\]\(\)\.\:\|]', "", raw_days).strip('-').strip()
        # days = ' '.join(raw_days.replace('(', '').replace('|', '').replace(')','').replace(',', '').replace('.', '').replace(':','').split()).strip('-').strip()
        # print days
...
Sunday: 11a-11p"""
    ...
    try:
        ...
        pass
        # output_csv.writerow([' '.join(value.split()).encode('ascii', 'ignore'), y.encode('ascii', 'ignore'), ' '.join(not_matched.split())])
    # cursor.close()
    # cnx.close()


print y
# accuracy_csv = csv.reader(open('store_hours.csv', 'rb'))
