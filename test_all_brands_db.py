...
replacements = {
    ...
    '[]': ['closed - closed', 'closed-closed', 'close - close', 'close-close','closed:closed', 'closed', 'close'],
...
def main_test():
    brand_name = '144_brands'
    query = """
       SELECT brand_name, raw_business_hours
       FROM O_O_DATA.shared_brands_hoo
       """
    cursor.execute(query)

    output_csv = csv.writer(open( "/media/nitin/809E47539E4740C0/HD/TestCodes/hours_of_operation/testing/compiled_code_us/test_all_brands/" + brand_name + ' store_hours.csv', 'wb'))
    output_csv.writerow(['brand_name','input_string', 'output_dict', "unmatched"])
    ...
    for value in cursor:
        ...
        output_csv.writerow([brand_name, ' '.join(value.split()).encode('ascii', 'ignore'), y, ' '.join(not_matched.split())])
        # output_csv.writerow([brand_name, store_name, store_type, address_1, city, state, zipcode, country_code,
        ...
        # break
def accuracy_csv():
        accuracy_csv = csv.reader(open("144_brands store_hours.csv", 'rb'))
        accuracy_csv = filter(None,[x for x in accuracy_csv][1:])

        total  = len([x for x in accuracy_csv])
        print ("Total rows %d" % total)

        unhandled = len([x for x in accuracy_csv if  'Unhandled' in x[2]  or 'invalid' in x[2] ])
        per_accuracy = ((total - unhandled) * 100)/total
        # print per_accuracy
        print ("Unhandled %d" % unhandled)
        print ("Accuracy  %d" % per_accuracy)

        faulty_brands = len(set([x[0] for x in accuracy_csv if  'Unhandled' in x[2]  or 'invalid' in x[2] ]))
        ...
        for row in accuracy_csv:
            if 'Unhandled' in row[2] or 'invalid' in row[2]:
